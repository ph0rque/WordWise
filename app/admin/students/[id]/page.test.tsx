...
describe('StudentDetailPage', () => {
  ...
  describe('Writing Sessions Tab', () => {
    ...
    it('switches to writing sessions tab', async () => {
      render(<StudentDetailPage />)
      
      await waitFor(() => {
        const sessionsTab = screen.getByText('Writing Sessions')
        fireEvent.click(sessionsTab)
      })
      
      expect(screen.getByText('Writing Sessions')).toBeInTheDocument()
      expect(screen.getByText('Detailed keystroke recordings and writing analytics')).toBeInTheDocument()
    })

    it('displays mock writing sessions', async () => {
      render(<StudentDetailPage />)
      ...
      // Should show mock sessions based on documents
      expect(screen.getAllByText('Playback')).toHaveLength(2)
    })

    it('handles playback action', async () => {
      const consoleSpy = jest.spyOn(console, 'log').mockImplementation()
      ...
      await waitFor(() => {
        ...
        const playbackButtons = screen.getAllByText('Playback')
        fireEvent.click(playbackButtons[0])
      })
      
      expect(consoleSpy).toHaveBeenCalledWith('View writing session:', expect.any(String))
      consoleSpy.mockRestore()
    })
  })
